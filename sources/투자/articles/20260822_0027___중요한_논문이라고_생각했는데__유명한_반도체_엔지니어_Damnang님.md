# * 중요한 논문이라고 생각했는데, 유명한 반도체 엔지니어 Damnang님

**수집일**: 2026-08-22

* 중요한 논문이라고 생각했는데, 유명한 반도체 엔지니어 Damnang님이 관련 아티클을 작성


두줄 요약 :
- 향후 HBM의 스택 쌓기보다, 계층화된 외부 메모리 풀링을 광학으로 연결하여 인터커넥트 병목을 줄이는 방향성
- 엔비디아, SK하이닉스, 마벨의 연구 방향성이 만나는 지점



"HBM 밀도가 높아질수록 시스템은 추가 용량 단위당 더 많은 비용을 지불해야 합니다.

이와 대조적으로 더 넓은 스케일업 영역에서는 해당 용량의 일부를 동료 HBM 및 패키지 외부의 메모리 계층으로 이동할 수 있습니다. 추가 로컬 용량의 가치와 이를 얻기 위한 패키지 비용 간의 격차가 좁아지면 메모리 밀도를 더 높이는 것보다 데이터 이동을 개선하는 것이 시스템 효율성을 더 높일 수 있습니다."


"SK하이닉스의 사업 범위는 DRAM 다이 자체에서 더 광범위한 메모리 아키텍처로 확장됩니다.

HBM과 맞춤형 베이스 다이는 로컬 고속 계층에, CXL과 CMM-Ax는 외부 용량 계층에, 그리고 논문에 언급된 광 메모리 풀은 더 오래된 광 계층에 위치합니다.

공개된 제품과 논문 한 편만으로는 시스템 레벨 메모리 공급업체로의 전환을 확인하기에 충분하지 않지만, 방향은 분명합니다. 고객과 공동 설계해야 하는 범위가 메모리 다이에서 컨트롤러, 베이스 다이, 패키징 및 메모리 배치로 확대됩니다. 이러한 구조가 상용화된다면 SK하이닉스가 확보하는 부가가치와 고객 충성도는 단순히 비트를 판매하는 것만으로는 얻을 수 없는 수준을 넘어설 수 있습니다."


"제가 계속해서 광학(optical) 투자에 두 번째 사이클이 올 것이라고 이야기하는 이유가 바로 이것입니다. 그리고 제가 계속해서 광학 테마의 시작이 메모리 테마가 강하게 움직이는 근본적인 이유라고 말하는 것도 같은 맥락입니다."




https://x.com/i/status/2090484296918478859

[링크 내용: https://x.com/i/status/2090484296918478859]
Damnang
@damnang2
HBM Density Peak: Why AI Value Is Moving From Memory to Connectivity
This article contains important information on the technical direction that will be highly relevant to future semiconductor, memory, and optics investments. Given its importance, I am making the entire article available for free.
I will share more details related to this article in my Substack thread. If you are interested, please join me on Substack.
